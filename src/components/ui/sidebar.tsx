  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'peer hidden transition-[width] md:block text-sidebar-foreground',
          state === 'collapsed'
...
>(
  (
    ...
    if (state === 'expanded') {
       ...
       return <button ref={ref} {...props} className={cn(sidebarMenuButtonVariants({ variant, size }), state === "collapsed" && "justify-center", className)}>{Icon}<span className={cn(state === 'collapsed' && 'hidden')}>{rest}</span></button>
    }


    const content = typeof tooltip === 'string' ? tooltip : tooltip.children;
